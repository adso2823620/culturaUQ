...
function labelAcceso(tipo: string) {
  const map: Record<string, string> = {
    gratuito: 'Gratuito', con_costo: 'Con costo',
    aporte_voluntario: 'Aporte', solo_inscripcion: 'Con inscripción',
  };
...
function getTipoEmoji(tipo: string) {
  const map: Record<string, string> = {
    taller: '🛠️', conversatorio: '🗣️', feria: '🎪', festival: '🎉',
    proceso_formacion: '📚', jornada_comunitaria: '🤝', lanzamiento: '🚀',
    seminario: '🎓', exposicion: '🖼️', presentacion_artistica: '🎭', otro: '🎨',
  }
  return map[tipo] ?? '🎨'
}
  ...
  useEffect(() => {
    async function fetchEventos() {
      ...
      if (error) { setEventos([]); }
        ...
        setEventos(lista);
        setAreas([...new Set(lista.map(e => e.area_artistica).filter(Boolean))] as string[]);
        setMunicipios([...new Set(lista.map(e => e.municipio).filter(Boolean))] as string[]);
      }
      setLoading(false);
    }
    fetchEventos();
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="flex flex-wrap gap-2 mb-8 p-1 bg-gray-50 rounded-2xl border border-gray-100 w-fit">
        {TABS.map(tab => (
          <Link
            ...
            style={{
              backgroundColor: tab.slug === 'agenda' ? '#e63947' : 'transparent',
              color: tab.slug === 'agenda' ? 'white' : '#6b7280',
            }}
          >
            {tab.label}
          </Link>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#e63947]/20 p-5 mb-8">
        <div className="flex items-center justify-between mb-5">
          <button onClick={mesAnterior} className="flex items-center gap-1 text-sm text-[#2a9d8f] hover:text-[#e63947] transition-colors px-3 py-1.5 rounded-lg hover:bg-[#e63947]/5">
            <ChevronLeft className="w-4 h-4" /> Anterior
          </button>
          <h2 className="text-xl text-[#2a9d8f] font-medium">{MESES_LARGO[mes]} {anio}</h2>
          <button onClick={mesSiguiente} className="flex items-center gap-1 text-sm text-[#2a9d8f] hover:text-[#e63947] transition-colors px-3 py-1.5 rounded-lg hover:bg-[#e63947]/5">
            Siguiente <ChevronRight className="w-4 h-4" />
          </button>
        </div>
        <div className="flex flex-wrap gap-3 items-center">
          {areas.length > 0 && (
            <select value={filtroArea} onChange={e => setFiltroArea(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none focus:border-[#2a9d8f]">
              <option value="todos">Todas las áreas</option>
              {areas.map(a => <option key={a} value={a}>{a}</option>)}
            </select>
          )}
          {municipios.length > 0 && (
            <select value={filtroMunicipio} onChange={e => setFiltroMunicipio(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none focus:border-[#2a9d8f]">
              <option value="todos">Todos los municipios</option>
              {municipios.map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          )}
          <select value={filtroModalidad} onChange={e => setFiltroModalidad(e.target.value)} className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none focus:border-[#2a9d8f]">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer ml-auto">
            <input type="checkbox" checked={soloGratis} onChange={e => setSoloGratis(e.target.checked)} className="rounded" />
            Solo gratuitos
          </label>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {eventosFiltrados.map((evento) => (
            <Card key={evento.id} onClick={() => setEventoSeleccionado(evento)}
              ...
              <div className="relative h-44 overflow-hidden">
                {evento.imagen_url ? (
                  <Image src={evento.imagen_url} alt={evento.titulo} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                ) : (
              ...
              <CardContent className="p-5">
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {evento.area_artistica && <span className="px-2 py-0.5 bg-[#0f4c75]/10 text-[#0f4c75] rounded-md text-xs">{evento.area_artistica}</span>}
                  {evento.tipo_evento && <span className="px-2 py-0.5 bg-[#e63947]/10 text-[#e63947] rounded-md text-xs capitalize">{evento.tipo_evento.replace(/_/g, ' ')}</span>}
                </div>
                <h3 className="text-base font-semibold text-[#2a9d8f] mb-2 group-hover:text-[#e63947] transition-colors line-clamp-2 leading-snug">{evento.titulo}</h3>
                {evento.descripcion && <p className="text-xs text-[#6B7280] mb-3 line-clamp-2 leading-relaxed">{evento.descripcion}</p>}
                <div className="space-y-1.5 text-xs text-[#6B7280]">
                  <div className="flex items-center gap-2">
                    <Calendar className="w-3.5 h-3.5 text-[#e63947] flex-shrink-0" />
                    {formatFechaCorta(evento.fecha_inicio)}
                    {evento.hora_inicio && <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-[#e63947]" />{formatHora(evento.hora_inicio)}</span>}
                  </div>
                  {evento.modalidad !== 'virtual' && evento.municipio && (
                    <div className="flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5 text-[#e63947] flex-shrink-0" />
                      <span className="line-clamp-1">{evento.lugar_nombre ? `${evento.lugar_nombre}, ` : ''}{evento.municipio}</span>
                    </div>
                  )}
                ...
                <div className="mt-4 text-[#e63947] text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300">Ver detalles completos →</div>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-2xl text-[#2a9d8f] mb-2">No hay eventos programados</h3>
          <p className="text-[#6B7280] mb-5">
            {eventos.length > 0 ? 'Intenta con otros filtros.' : `Aún no hay eventos publicados para ${MESES_LARGO[mes]} ${anio}.`}
          </p>
          {eventos.length > 0 && (
            <button onClick={() => { setFiltroArea('todos'); setFiltroMunicipio('todos'); setFiltroModalidad('todos'); setSoloGratis(false); }}
      ...
      {eventoSeleccionado && <EventoModal evento={eventoSeleccionado} onClose={() => setEventoSeleccionado(null)} />}
    </div>
  );
}
